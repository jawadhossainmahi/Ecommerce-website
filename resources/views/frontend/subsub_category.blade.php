        ...
        <ol class="inline-flex flex-wrap items-center space-x-1 md:space-x-3">
          ...
          <li>
            <div class="flex items-center">
              ...
              <a href="{{ env("BASE_URL") }}product/category?category={{ $subsub_category[0]->getsubcategory->getcategory->id }}" class="ml-1 text-sm font-medium text-gray-700 hover:text-blue-600 md:ml-2 dark:text-gray-400 dark:hover:text-white">{{ $subsub_category[0]->getsubcategory->getcategory->name }}</a>
            </div>
          </li>
          <li aria-current="page">
            <div class="flex items-center">
              <svg aria-hidden="true" class="w-6 h-6 text-gray-400" fill="currentColor" viewBox="0 0 20 20"><path fill-rule="evenodd" d="M7.293 14.707a1 1 0 010-1.414L10.586 10 7.293 6.707a1 1 0 011.414-1.414l4 4a1 1 0 010 1.414l-4 4a1 1 0 01-1.414 0z" clip-rule="evenodd"></path></svg>
              <a href="{{ env("BASE_URL") }}product/sub_cat/{{ $subsub_category[0]->getsubcategory->id }}" class="ml-1 text-sm font-medium text-gray-500 md:ml-2 dark:text-gray-400">{{ $subsub_category[0]->getsubcategory->name }}</a>
            </div>
          </li>
        </ol>
      </div>
    ...
    <section class="fruit-and-vegitable">
        <div class="max-w-7xl mx-auto px-3">
            ...
            <div class="main-heading">
                <h1 class="text-left"> {{ $subsub_category[0]->name }}</h1>
            </div>
...
<!-- -------------------filter----------------- -->
    ...
    <section class="card-filter overflow-hidden mx-0">
        <div class="max-w-7xl mx-auto px-3 py-4">
            ...
            <div class="row">
                <div class="catagories col-lg-3 d-none d-lg-block" style="margin-top: 10px;position: relative">


                  @if(!$sub_category->isEmpty())
                        @foreach ($sub_category[0]->getsubsubcategory as $item)
                ...
                <div id="subsub_category" class=" row col-12 col-lg-9 mx-auto ">
                ...
                @if($request->sub_sub_category)
                      ...
                      {{-- <div style="text-align:center"> --}}
                        <a class="btn btn-success my-4 LoadMore hidden" id="SubSubCategoryLoadMore" data-id="{{$request->sub_sub_category}}"  >Ladda mer</a>
                {{-- </div > --}}
                @endif
                {{-- @endif --}}
                </div>
        ...
        @if( $request->sub_sub_category)
        <input type="hidden" id="page-id" value="{{ $request->sub_sub_category }}"  >
        @endif
 
<div id="subsub-category-modal"></div>
